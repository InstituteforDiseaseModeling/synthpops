...
class Pop(sc.prettyobj):
    ...
    def __init__(self,
        ...
        Args:
            ...
            use_default (bool)                      : If True, use default data from default_config.location, default_config.state, default_config.country.
        ...
        if self.country_location is None:
            self.country_location = defaults.default_config.country_location
            self.state_location   = defaults.default_config.state_location
            self.location         = defaults.default_config.location
        else:
        ...
        if self.sheet_name is None:
            self.sheet_name = defaults.default_config.sheet_name
        self.datadir = defaults.default_config.datadir  # Assume this has been reset...
    ...
    def generate(self):
        ...
        hha_by_size = spdata.get_head_age_by_size_distr(datadir, country_location=country_location, state_location=state_location, use_default=use_default, 
            # household_size_1_included=defaults.default_config.household_size_1_included
            )
    ...
    def enrollment_rates_by_age(self):
        ...
        """
        return {k: self.summary.enrollment_by_age[k]/self.summary.age_count[k] if self.summary.age_count[k] > 0 else 0 for k in range(defaults.default_config.max_age)}
    ...
    def employment_rates_by_age(self):
        ...
        """
        return {k: self.summary.employment_by_age[k]/self.summary.age_count[k] if self.summary.age_count[k] > 0 else 0 for k in range(defaults.default_config.max_age)}
