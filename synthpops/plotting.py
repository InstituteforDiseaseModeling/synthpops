...
class plotting_kwargs(sc.objdict):
    ...
    def set_default_pop_pars(self):
        ...
        """
        default_pop_pars = sc.objdict(datadir=spd.settings_config.datadir, location=spd.settings_config.location, state_location=spd.settings_config.state_location,
                                      country_location=spd.settings_config.country_location, use_default=False)
...
def plot_contacts(pop, **kwargs):
    ...
    method_defaults = sc.objdict(layer='H', density_or_frequency='density',
                                 ...
                                 state_location=spd.settings_config.state_location, country_location=spd.settings_config.country_location
                                 )
    method_defaults.figname = f"contact_matrix_{method_defaults.layer}"  # by defining this here, we can at least ensure that default names connect to the layer being modeled
...
def plot_employment_rates_by_age(pop, **kwargs):
    ...
    **Example**::
    ...
    expected_employment_rates_by_age = dict.fromkeys(np.arange(spd.settings_config.max_age), 0)
...
def plot_school_sizes(pop, **kwargs):
    ...
    **Example**::
    ...
    if plkwargs.location is not None:
        ...
        location_text = f"{spd.settings_config.location.replace('_', ' ').title()}"
